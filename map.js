(function(){function _$rapyd$_extends(child,parent){child.prototype=new parent;child.prototype.constructor=child}function reversed(arr){var tmp=[];for(var i=arr.length-1;i>=0;i--){tmp.push(arr[i])}return tmp}function range(start,stop,step){if(arguments.length<=1){stop=start||0;start=0}step=arguments[2]||1;var length=Math.max(Math.ceil((stop-start)/step),0);var idx=0;var range=new Array(length);while(idx<length){range[idx++]=start;start+=step}return range}function len(obj){if(obj instanceof Array||typeof obj==="string")return obj.length;else{var count=0;for(var i in obj){if(obj.hasOwnProperty(i))count++}return count}}function enumerate(item){var arr=[];for(var i=0;i<item.length;i++){arr[arr.length]=[i,item[i]]}return arr}var JSON,str;JSON=JSON||{};if(!JSON.stringify){
	...
	Array.prototype.filter = function(filterfun, thisArg) {
        ...
        ;return keys}}else{dict.keys=function(hash){return Object.getOwnPropertyNames(hash)}}dict.values=function(hash){var vals,key;vals=[];var _$rapyd$_Iter3=dict.keys(hash);for(var _$rapyd$_Index3=0;_$rapyd$_Index3<_$rapyd$_Iter3.length;_$rapyd$_Index3++){key=_$rapyd$_Iter3[_$rapyd$_Index3];vals.append(hash[key])}return vals};dict.items=function(hash){var items,key;items=[];var _$rapyd$_Iter4=dict.keys(hash);for(var _$rapyd$_Index4=0;_$rapyd$_Index4<_$rapyd$_Iter4.length;_$rapyd$_Index4++){key=_$rapyd$_Iter4[_$rapyd$_Index4];items.append([key,hash[key]])}return items};dict.copy=dict;dict.clear=function(hash){var key;var _$rapyd$_Iter5=dict.keys(hash);for(var _$rapyd$_Index5=0;_$rapyd$_Index5<_$rapyd$_Iter5.length;_$rapyd$_Index5++){key=_$rapyd$_Iter5[_$rapyd$_Index5];delete hash[key]}};function makeMap(){var map,tilesURL,BINS,LABELS,COLORS,MEDIAN_ANNUAL_INCOME,suburbs,info,legend,item;map=L.map("map").setView([-36.84041,174.73986],11);tilesURL="http://{s}.tiles.mapbox.com/v3/github.map-xgq2svrz/{z}/{x}/{y}.png";L.tileLayer(tilesURL).addTo(map);map.attributionControl.setPrefix("");BINS=[0,.25,.5,.75,1e10];LABELS=["n/a","0&ndash;25%","26&ndash;50%","51&ndash;75%","76%+"];COLORS=reversed(["#d7191c","#fdae61","#abdda4","#2b83ba","#aaa"]);MEDIAN_ANNUAL_INCOME=882*52;suburbs=null;info=L.control();info.onAdd=function(map){this._div=L.DomUtil.create("div","info");this.update();return this._div};info.update=function(feature){if(typeof feature==="undefined")feature=null;var message;if(feature){message="<h4>"+feature.properties.AU_NAME+"</h4>"+"Weekly rent per bedroom: "+getWeeklyRent(feature,0,true)+"<br>"+"Fraction of annual income: "+getRentFraction(feature,2,true)+"<br>"}else{message="<h4>Suburb Info</h4>Hover over a suburb"}this._div.innerHTML=message};info.addTo(map);legend=L.control({position:"bottomright"});legend.onAdd=function(map){var div,i;div=L.DomUtil.create("div","info legend");var _$rapyd$_Iter6=reversed(range(len(LABELS)));for(var _$rapyd$_Index6=0;_$rapyd$_Index6<_$rapyd$_Iter6.length;_$rapyd$_Index6++){i=_$rapyd$_Iter6[_$rapyd$_Index6];div.innerHTML+='<i style="background:'+getColor(BINS[i])+'"></i> '+LABELS[i]+"<br>"}return div};legend.addTo(map);function getGrossAnnualIncome(string){if(typeof string==="undefined")string=false;var result;result=$("#income").val();if(!string){result=numToDollarStr(result,0,true)}return result}function getWeeklyRent(feature,ndigits,string){if(typeof ndigits==="undefined")ndigits=0;if(typeof string==="undefined")string=false;var propertyType,numBedrooms,result;propertyType=$("#property-type").val();numBedrooms=$("#num-bedrooms").val();if(feature.properties.rent_by_nbedrooms_by_property!="NA"){result=parseFloat(feature.properties.rent_by_nbedrooms_by_property[propertyType][numBedrooms])}else{result=-1}if(result>=0){if(string){result=numToDollarStr(result,ndigits,false)}}else{if(string){result="n/a"}else{result=-1}}return result}function getRentFraction(feature,ndigits,string){if(typeof ndigits==="undefined")ndigits=2;if(typeof string==="undefined")string=false;var result;result=52*getWeeklyRent(feature,ndigits,false)/getGrossAnnualIncome();if(result>=0){if(string){result=Math.ceil(result*100)+"%"}else{result=parseFloat(result.toFixed(ndigits))}}else{if(string){result="n/a"}}return result}function numToDollarStr(x,ndigits,inverse){if(typeof ndigits==="undefined")ndigits=0;if(typeof inverse==="undefined")inverse=false;var y,_$rapyd$_Unpack,cents,dollars;if(!inverse){y=x.toFixed(ndigits);_$rapyd$_Unpack=y.split(".");dollars=_$rapyd$_Unpack[0];cents=_$rapyd$_Unpack[1];dollars=dollars.replace(/\B(?=(\d{3})+(?!\d))/g,",");if(ndigits){return"$"+dollars+"."+cents}else{return"$"+dollars}}else{return parseInt(x.replace("$","").replace(",",""))}}function getColor(x){var _$rapyd$_Unpack,i,grade;var _$rapyd$_Iter7=enumerate(BINS);for(var _$rapyd$_Index7=0;_$rapyd$_Index7<_$rapyd$_Iter7.length;_$rapyd$_Index7++){_$rapyd$_Unpack=_$rapyd$_Iter7[_$rapyd$_Index7];i=_$rapyd$_Unpack[0];grade=_$rapyd$_Unpack[1];if(x<=grade){return COLORS[i]}}}function style(feature){var c;c=getColor(getRentFraction(feature));return{"fillColor":c,"fillOpacity":.7,"color":c,"weight":1,"opacity":.8}}function highlightFeature(e){var layer;layer=e.target;layer.setStyle({"weight":8,"opacity":1});if(!L.Browser.ie&&!L.Browser.opera){layer.bringToFront()}info.update(layer.feature)}function resetHighlight(e){var layer;layer=e.target;suburbs.resetStyle(layer);info.update()}function zoomToFeature(e){map.fitBounds(e.target.getBounds())}function onEachFeature(feature,layer){layer.on({"mouseover":highlightFeature,"mouseout":resetHighlight,"click":zoomToFeature})}$.getJSON("data/AU_TA_Auckland_CC_with_rents.geojson",function(collection){suburbs=L.geoJson(collection,{style:style,onEachFeature:onEachFeature}).addTo(map)});$(function(){var sv,min,range,el;$("#slider-vertical").slider({"orientation":"vertical","range":"min","min":100,"max":1e5,"value":MEDIAN_ANNUAL_INCOME,"step":100,"slide":function(event,ui){$("#income").val(numToDollarStr(ui.value));suburbs.setStyle(style)}});sv=$("#slider-vertical");$("#income").val(numToDollarStr(sv.slider("value")));min=sv.slider("option","min");range=sv.slider("option","max")-min;console.log("range=",range,MEDIAN_ANNUAL_INCOME/range*100);el=$('<label><span id="arrow">&larr;</span>'+"Median annual income<br>of employed Aucklanders<br>("+numToDollarStr(MEDIAN_ANNUAL_INCOME)+")</label>").css("bottom",MEDIAN_ANNUAL_INCOME/range*100+"%");$("#slider-vertical").append(el)});$(function(){$("#property-type").selectable({"selected":function(event,ui){$("#property-type").val(ui.selected.id);suburbs.setStyle(style)}})});item=$("#property-type li:eq(1)");item.addClass("ui-selected");$("#property-type").val(item[0].id);$(function(){$("#num-bedrooms").selectable({"selected":function(event,ui){$("#num-bedrooms").val(ui.selected.id);suburbs.setStyle(style)}})});item=$("#num-bedrooms li:eq(1)");item.addClass("ui-selected");$("#num-bedrooms").val(item[0].id)}makeMap();function f(x,kwarg1,kwarg2){if(typeof kwarg1==="undefined")kwarg1=true;if(typeof kwarg2==="undefined")kwarg2="hello";return str(x)+str(kwarg1)+kwarg2}console.log("=================================");console.log(f(1));console.log(f(1,kwarg2="bye"));console.log(f(1,kwarg1=false));console.log(f(1,kwarg1=false,kwarg2="yowza"));console.log(f(1,kwarg2="yowza",kwarg1=false))})();