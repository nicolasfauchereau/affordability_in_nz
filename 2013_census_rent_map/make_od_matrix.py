...
def get_od_matrix(filename, name_field, n=NUM_SAMPLE_POINTS):
    ...
    num_polygons = len(names)
    for i in range(num_polygons):
        p_name = names[i]
        p_centroid = point_to_tuple(pc_by_name[p_name][1])
        a = wgs84_to_nztm(*p_centroid, inverse=True)
        for j in range(i + 1, num_polygons):
            # Compute the distance from the centroid of aus[i] to 
            # the centroid of aus[j]
            # along the quickest route in the Open Stree Map network.
            # Also get the time for that route.
            q_name = names[j]
            q_centroid = point_to_tuple(pc_by_name[q_name][1])
            b = wgs84_to_nztm(*q_centroid, inverse=True)
            M[p_name][q_name] = get_osm_distance_and_time(a, b)
    for p_name in names:
